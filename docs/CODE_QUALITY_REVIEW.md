...
主要短板集中在工程化边界：配置密钥安全、旧版和新版 Agent 并存导致入口认知成本较高，以及 CLI/Agent 工具链测试覆盖仍需继续补强。持久化层已默认切换到 SQLite，并保留 JSON 后端作为兼容迁移入口。
...
- 分层基本清晰：领域层不依赖 UI 或 LLM，`ToolExecutor` 作为 Agent 工具边界，LLM 客户端单独封装后端。
- 工具调用链路更稳定：Pydantic 参数模型同时约束 Tool Calling Schema 和本地执行前校验。
- 本地可测试性较好：JSON/SQLite 数据层、LLM 适配层和 Agent 校验重试已有 `unittest` 覆盖，且测试不依赖真实网络。
- 兼容性意识明确：`Todo.due_date` 保留旧字段别名，SQLite 空库可从旧版 `todos.json` 迁移。
- 用户入口完整：CLI 和 GUI 默认使用同一个 SQLite 仓储工厂。
...
| 已处理 | JSON 持久化不是原子写 | 默认后端已切换为 `SQLiteTodoRepository`，JSON 后端仅用于兼容和回退 | 降低进程中断或并发运行 CLI/GUI 时的数据损坏风险 |
...
4. 继续收敛旧版 JSON Agent：如果保留，文档标注“旧版演示”；如果不保留，逐步删除重复逻辑。

## 维护约定建议

- 新增业务能力优先从 `TodoRepository` 端口和对应仓储实现增加可测试方法，再通过 `ToolExecutor` 暴露给 Agent。
- 新增自然语言能力时同步更新 `TOOL_DEFINITIONS`、`ToolExecutor` 和至少一个 `AgentCore` fake-client 测试。
- CLI 展示逻辑不要反向写入数据规则；数据规则应沉到 `Todo`、应用服务或仓储实现。
