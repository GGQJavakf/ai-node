...
- 经用户现场授权的真实 Obsidian preview、apply、单事件 retry 和读回已经完成；提交版仅保留脱敏结论，不保留机器路径、现场 ID 或内容哈希。
- 仓库级 Ruff 仍有十二个已确认的历史告警，和本实现无关；按用户选择放入后续独立清理提交，不影响本次增量文件 Ruff 门。
- 两条真实 pending 候选必须在初始化和投影重试前后保持同 ID、同状态。

### Action

真实写入闭环无需重复执行。后续只需从最终干净提交重建 runtime，确认安装清单 `source_dirty=false`，再进入独立 Ruff 清理批次。

### 真实 Obsidian 回读

- Preview plan：只包含一个已存在的项目摘要目标，仅在文件末尾追加空 summary block；不存在的项目索引未列入写入。
- Apply readback：项目页输出与计划一致，marker 数为 2；保留备份与原输入一致；初始化 sync job 为 `synced`。
- Retry readback：已知 projection event 与 sync job 均为 `synced`；摘要为规则 0、经验 0、任务状态 1；原项目页完整字节仍是当前文件的前缀。
- 生成文件：`任务状态.md` 与 `变更日志.md` 存在，事件 marker 恰好 1 个；没有规则/经验知识，因此对应文件未创建；项目索引仍不存在。
- 边界回读：两条既有候选仍为 `pending_review`；全局 `AGENTS.md` 内容哈希保持不变。
