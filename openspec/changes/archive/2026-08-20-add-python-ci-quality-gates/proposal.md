...
- Correct the reproduced cross-runtime baseline defects in Windows shim path resolution, invalid `/sync` project-path handling, Python-version-sensitive candidate-budget assertions, localized timestamp formatting, and coarse-clock candidate ordering without weakening their intended safety guarantees.
